class PythonAT36 < Formula
  ...
  def lib_cellar
    prefix/"Frameworks/Python.framework/Versions/3.6/lib/python3.6"
  end
  ...
  def install
    ...
    # Any .app get a " 3.6" attached, so it does not conflict with python 2.x and 3.x.
    ...
    rm bin/"2to3"
    # MYN other stuff to remove that results in link error
    rm bin/"idle3"
    rm bin/"pydoc3"
    rm bin/"python3"
    rm bin/"python3-config"
    rm bin/"pyvenv"
    rm share/"man/man1/python3.1"
